...
class AclAccessTable extends Table {
    ...
    public function initialize(array $config):void{
        $this->setTable('acl_access');
        $this->setPrimaryKey(['acl_aro_id','acl_aco_id','acl_alo_id']);
        $this->belongsTo('Base.AclAro');
        $this->belongsTo('Base.AclAco');
        $this->belongsTo('Base.AclAlo');
        $this->setEntityClass('\\Base\Model\\Entity\\AclAccess');
    }

    public function mask($aroId,$acoId,$aloId){
        ...
        $query=$query->where([$this->getAlias().'.acl_aco_id'=>$acoId]);
    ...
    public function check($aroId,$acoId,$aloId,int $mask=null,bool $full=false){
        /** @var \Cake\ORM\Query $query */
        $query=$this->find();
